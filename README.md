...
- Not stored in the save: the name of some injury types. A save's own injury-name table has no entry for a few of the codes its players' injuries carry, so those rows come back with an empty name; fmsave ships no injury names of its own, as it ships no competition names.
