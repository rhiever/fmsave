...
- `export` writes one table (`players`, `contracts`, `suspensions`, `clubs`, `managed-clubs`, `stages`, `competitions`, `fixtures`, `league-tables`, `transfer-windows`, `competition-rules` or `player-match-stats`) as CSV, JSON or JSON Lines (`--format csv|json|jsonl`), to standard output or to a file with `-o`. `--columns` picks and orders columns.
